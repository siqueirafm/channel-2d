...
namespace channel {
  ...
  /**
   * \fn CurveBuilder::CurveBuilder( size_t np , size_t nc , bool closed , double* lx , double* ly , double* ux , double* uy ) throw( ExceptionObject ) 
   *
  ...
  CurveBuilder::build( int& error )
  {
    ...
    // Compute the linear constraints that ensure that the breakpoints
    // of the sleeves belong to the  channel, but not the sleeves as a
    // whole!
    compute_sleeve_corners_in_channel_constraints( eqline ) ;

    // Compute the linear constraints  that ensure that channel cornes
    // do not intersect the sleeves. Together with the previous set of
    // constraints, these  constraints ensure that the  sleeves belong
    // to the channel.
  ...
  /**
   * \fn size_t CurveBuilder::compute_barycentric_coordinate_column_index( const size_t i ) const
   *
   * \brief Computes  the index  of the  linear program  matrix column
   ...
   * \return   The  index   of  the   linear  program   matrix  column
   * corresponding   to  the   barycentric  coordinate   defining  the
   * \f$i\f$-th endpoint of the b-spline with respect to the endpoints
   * of the channel.
   *
   */
  size_t
  CurveBuilder::compute_barycentric_coordinate_column_index( const size_t i ) const
  {
#ifdef DEBUGMODE
    if ( _closed ) {
      assert( i == 0 ) ;
  ...
  CurveBuilder::compute_correspondence_constraints( size_t& eqline )
  {
    ...
    strx[ 3 ] = compute_barycentric_coordinate_column_index( 0 ) ;
    ...
    if ( _closed ) {
      ...
      endx[ 3 ] = compute_barycentric_coordinate_column_index( 1 ) ;
  ...
  CurveBuilder::compute_sleeve_corners_in_channel_constraints( size_t& eqline )
  {
    ...
    for ( size_t c = 0 ; c < NumberOfCSections ; c++ ) {
      compute_normal_to_csection( c , ncsec[ c ][ 0 ] , ncsec[ c ][ 1 ] ) ;
    }
  ...
  /**
   * \fn void CurveBuilder::insert_nonlinear_terms_of_epiece_point_lower_bound( const size_t eqline , const double s , const size_t c , const std::vector< std::vector< std::vector< size_t > > >& sd , const std::vector< std::vector< double > >& nl , const std::vector< std::vector< double > >& nu , const std::vector< std::vector< double > >& ncsec )
   *
   ...
   * channel.
   * \param ncsec  Array of  Cartesian coordinates  of normals  to the
   * supporting  lines  (pointing  left)  of  the  c-sections  of  the
   * channel.
   * 
   */
  void
                                                                  ...
                                                                  )
  {
    ...
    for ( size_t j = 1 ; j < 3 ; j++ ) {
      ...
      for ( size_t v = 0 ; v < 2 ; v++ ) {
        ...
        // Point \f$\stackrel{e}{\sim}^p( s )  \f$ of the e-piece must
        // be on  or on  the left  side of the  c-th c-section  of the
        // channel.
        insert_csegment_constraint(
                                   eqline + 2 ,
                                   dl ,
        ...
        // Point \f$\stackrel{e}{\sim}^p( s )  \f$ of the e-piece must
        // be on  or on  the right  of the  (c+1)-th c-section  of the
        // channel.
        insert_csegment_constraint(
                                   eqline + 3 ,
                                   dl ,
                                   ...
                                   sd[ j - 1 ][ 0 ][ v ] ,
                                   sd[ j - 1 ][ 1 ][ v ] ,
                                   ( _closed ) ? ncsec[ ( c + 1 ) % _nc ][ v ] : ncsec[ c + 1 ][ v ]
                                  ) ;
      }
    }
  ...
  /**
   * \fn void CurveBuilder::insert_nonlinear_terms_of_epiece_point_lower_bound( const size_t eqline , const double s1 , const double s2 , const double u ,const size_t c , const std::vector< std::vector< std::vector< size_t > > >& sd1 , const std::vector< std::vector< std::vector< size_t > > >& sd2 , const std::vector< std::vector< double > >& nl , const std::vector< std::vector< double > >& nu )
   *
   * \brief Inserts the coefficients of the second difference terms of
   * the equation defining lower bounds for a given e-piece point into
   * the  matrix associated  with an  instance of  the Linear  Program
   * (LP).  The  e-piece point belongs  to a e-piece segment  given ny
   * its two  e-piece endpoints.  The terms  belong to  the constraint
   * that forces the the channel corners to stay outside the sleeve.
   *
   * \param eqline A counter for the number of constraints.
   * \param s1 A parameter value identifying an endpoint of an e-piece
   * segment.
   * \param s2  A parameter value  identifying another endpoint  of an
   * e-piece segment.
   * \param u  A parameter  value identifying a  point on  the e-piece
   * segment defined by  the two e-piece endpoints given by  \e s1 and
   * \e s2.
   * \param c An index identifying  a c-segment containing the e-piece
   * point at parameter \e u.
   * \param sd1 Array with the  LP matrix column indices corresponding
   * to the lower and upper  bounds on second differences occurring in
   * the equation defining the the e-piece point at parameter \e s1.
   * \param sd2 Array with the  LP matrix column indices corresponding
   * to the lower and upper  bounds on second differences occurring in
   * the equation defining the the e-piece point at parameter \e s2.
   * \param nl  Array of Cartesian  coordinates of outward  normals to
   * the  supporting  lines of  the  lower  envelope segments  of  the
   * channel.
   * \param nu  Array of Cartesian  coordinates of outward  normals to
  ...
  CurveBuilder::insert_nonlinear_terms_of_epiece_point_lower_bound(
                                                                   ...
                                                                   const double u ,
                                                                   const size_t c ,
                                                                   const std::vector< std::vector< std::vector< size_t > > >& sd1 ,
                                                                   const std::vector< std::vector< std::vector< size_t > > >& sd2 ,
                                                                   const std::vector< std::vector< double > >& nl ,
                                                                   const std::vector< std::vector< double > >& nu
                                                                  )
  {
    ...
    //
    // (a) endpoints of  the e-piece segment belong to  the same curve
    // segment ;
    //
    // (b) endpoints of  the e-piece segment belong  to distinct curve
    // segments ;
    //
    // ---------------------------------------------------------------

    if ( sd1[ 0 ][ 0 ][ 0 ] == sd2[ 0 ][ 0 ][ 0 ] ) {
      //
      // Case (a)
      //
      for ( size_t j = 1 ; j < 3 ; j++ ) {
        ...
        evaluate_bounding_polynomial(
                                     j  ,
                                     s1 ,
                                     duaux ,   // switch lower and upper bounds.
                                     dlaux     // switch lower and upper bounds.
                                    ) ;
        ...
        for ( size_t v = 0 ; v < 2 ; v++ ) {
          // Point
          //
          // \f$( 1 - u ) \cdot \stackrel{e}{\sim}^p( s_1 ) + u \cdot \stackrel{e}{\sim}^p( s_2 )\f$
          //
          // must be  above the  lower envelope  of the (  c -  1 )-th
          // c-segment of the  channel, which is the  c-segment on its
          // right.
          insert_csegment_constraint(
                                     eqline ,
                                     dl ,
          ...
          // \f$( 1 - u ) \cdot \stackrel{e}{\sim}^p( s_1 ) + u \cdot \stackrel{e}{\sim}^p( s_2 )\f$
          //
          // must  be on  or  above  the lower  envelope  of the  c-th
          // c-segment of the  channel, which is the  c-segment on its
          // left.
          insert_csegment_constraint(
                                     eqline + 1 ,
          ...
          // \f$( 1 - u ) \cdot \stackrel{e}{\sim}^p( s_1 ) + u \cdot \stackrel{e}{\sim}^p( s_2 )\f$
          //
          // must be  on or below  the lower envelope of  the ( c  - 1
          // )-th c-segment of the channel,  which is the c-segment on
          // its right.
          insert_csegment_constraint(
                                     eqline + 2 ,
                                     dl ,
          ...
          // \f$( 1 - u ) \cdot \stackrel{e}{\sim}^p( s_1 ) + u \cdot \stackrel{e}{\sim}^p( s_2 )\f$
          //
          // must  be on  or  below  the lower  envelope  of the  c-th
          // c-segment of the  channel, which is the  c-segment on its
          // left.
          insert_csegment_constraint(
                                     eqline + 3 ,
      ...
      //
      // Get lower and upper bounds for the bounding polynomial.
      //
      double dl1 ;
      double du1 ;
      evaluate_bounding_polynomial(
                                   1  ,
                                   s1  ,
                                   du1 ,   // switch lower and upper bounds.
                                   dl1     // switch lower and upper bounds.
                                  ) ;
      ...
      evaluate_bounding_polynomial(
                                   2  ,
                                   s1  ,
                                   du2 ,   // switch lower and upper bounds.
                                   dl2     // switch lower and upper bounds.
                                  ) ;
      ...
      evaluate_bounding_polynomial(
                                   1  ,
                                   s2  ,
                                   du3 ,   // switch lower and upper bounds.
                                   dl3     // switch lower and upper bounds.
                                  ) ;
      ...
      evaluate_bounding_polynomial(
                                   2  ,
                                   s2  ,
                                   du4 ,   // switch lower and upper bounds.
                                   dl4     // switch lower and upper bounds.
                                  ) ;
      ...
      for ( size_t v = 0 ; v < 2 ; v++ ) {
...
#ifdef DEBUGMODE
        assert( sd1[ 1 ][ 0 ][ v ] == sd2[ 0 ][ 0 ][ v ] ) ;
        assert( sd1[ 1 ][ 1 ][ v ] == sd2[ 0 ][ 1 ][ v ] ) ;
#endif
        ...
        // \f$( 1 - u ) \cdot \stackrel{e}{\sim}^p( s_1 ) + u \cdot \stackrel{e}{\sim}^p( s_2 )\f$
        //
        // must be on or above the lower  envelope of the ( c - 1 )-th
        // c-segment of  the channel,  which is  the c-segment  on its
        // right.
        insert_csegment_constraint(
                                   eqline + 1 ,
        ...
        // \f$( 1 - u ) \cdot \stackrel{e}{\sim}^p( s_1 ) + u \cdot \stackrel{e}{\sim}^p( s_2 )\f$
        //
        // must be on or below the lower  envelope of the ( c - 1 )-th
        // c-segment of  the channel,  which is  the c-segment  on its
        // right.
        insert_csegment_constraint(
                                   eqline + 2 ,
  ...
  /**
   * \fn void CurveBuilder::insert_nonlinear_terms_of_epiece_point_upper_bound( const size_t eqline , const double s , const size_t c , const std::vector< std::vector< std::vector< size_t > > >& sd , const std::vector< std::vector< double > >& nl , const std::vector< std::vector< double > >& nu , const std::vector< std::vector< double > >& ncsec )
   *
   * \brief Inserts the coefficients of the second difference terms of
   * the equation  defining lower bounds  for the e-piece  points into
   * the  matrix associated  with an  instance of  the Linear  Program
  ...
  /**
   * \fn void CurveBuilder::insert_nonlinear_terms_of_epiece_point_upper_bound( const size_t eqline , const double s1 , const double s2 , const double u ,const size_t c , const std::vector< std::vector< std::vector< size_t > > >& sd1 , const std::vector< std::vector< std::vector< size_t > > >& sd2 , const std::vector< std::vector< double > >& nl , const std::vector< std::vector< double > >& nu )
   *
   * \brief Inserts the coefficients of the second difference terms of
   * the equation defining upper bounds for a given e-piece point into
   * the  matrix associated  with an  instance of  the Linear  Program
   * (LP).  The  e-piece point belongs  to a e-piece segment  given ny
   * its two  e-piece endpoints.  The  terms belong to  the constraint
   * that forces the the channel corners to stay outside the sleeve.
    ...
    if ( sd1[ 0 ][ 0 ][ 0 ] == sd2[ 0 ][ 0 ][ 0 ] ) {
      ...
      for ( size_t j = 1 ; j < 3 ; j++ ) {
        ...
        evaluate_bounding_polynomial(
                                     j  ,
                                     s1 ,
                                     dlaux ,   // DON't switch lower and upper bounds.
                                     duaux     // DON't switch lower and upper bounds.
                                    ) ;
        ...
        for ( size_t v = 0 ; v < 2 ; v++ ) {
          // Point
          //
          // \f$( 1 - u ) \cdot \stackrel{\sim}{e}^p( s_1 ) + u \cdot \stackrel{\sim}{e}^p( s_2 )\f$
          //
          // must be  above the  lower envelope  of the (  c -  1 )-th
          // c-segment of the  channel, which is the  c-segment on its
          // right.
      ...
      evaluate_bounding_polynomial(
                                   1  ,
                                   s1  ,
                                   dl1 ,   // DON't switch lower and upper bounds.
                                   du1     // DON't switch lower and upper bounds.
                                  ) ;
      ...
      evaluate_bounding_polynomial(
                                   2  ,
                                   s1  ,
                                   dl2 ,   // DON't switch lower and upper bounds.
                                   du2     // DON't switch lower and upper bounds.
                                  ) ;
      ...
      evaluate_bounding_polynomial(
                                   1  ,
                                   s2  ,
                                   dl3 ,   // DON't switch lower and upper bounds.
                                   du3     // DON't switch lower and upper bounds.
                                  ) ;
      ...
      evaluate_bounding_polynomial(
                                   2  ,
                                   s2  ,
                                   dl4 ,   // DON't switch lower and upper bounds.
                                   du4     // DON't switch lower and upper bounds.
                                  ) ;
  ...
  /**
   * \fn void CurveBuilder::insert_linear_terms_of_epiece_point_bounds( const size_t eqline , const double s , const double t , const size_t p , const size_t c , const std::vector< std::vector< size_t > >& cp , const std::vector< std::vector< double > >& nl , const std::vector< std::vector< double > >& nu , const std::vector< std::vector< double > >& ncsec )
   *
                                                          ...
                                                          )
  {
    ...
    for ( size_t v = 0 ; v < 2 ; v++ ) {
      ...
      // The upper and  lower bounds on the e-piece points  must be on
      // or on the left of the  c-th c-section of the c-segment of the
      // channel.
      insert_csegment_constraint(
                                 eqline + 2 ,
                                 c0 ,
      ...
      // The upper and lower bounds on the e-piece points must be on
      // or on the right of the (c+1)1-th c-section of the c-segment
      // of the channel.
      insert_csegment_constraint(
                                 eqline + 3 ,
                                 c0 ,
  ...
  /**
   * \fn void CurveBuilder::insert_linear_terms_of_epiece_point_bounds( const size_t eqline , const double s1 , const double s2 , const double t1 , const double t2 , const size_t p1 , const size_t p2 , const double u , const size_t c , const std::vector< std::vector< size_t > >& cp1 , const std::vector< std::vector< size_t > >& cp2 , const std::vector< std::vector< double > >& nl , const std::vector< std::vector< double > >& nu )
   *
   * \brief  Inserts  the coefficients  of  the  linear terms  of  the
   * equation defining  lower and  upper bounds  for an  e-piece point
   * into  the matrix  associated with  the Linear  Program (LP).  The
   * point  belongs to  an e-piece  segment given  by its  two e-piece
   * endpoints.   The  linear  terms  belong to  the  constraint  that
   * enforces a channel corner to stay  on the boundary or outside the
   * sleeve.
   *
   * \param eqline A counter for the number of constraints.
   * \param s1 A parameter value identifying the right endpoint of the
   * e-piece segment.
   * \param s2 A parameter value  identifying the left endpoint of the
   * e-piece segment.
   * \param t1 A parameter value  identifying the b-spline curve point
   * that corresponds to the right endpoint of the e-piece segment.
   * \param t2 A parameter value  identifying the b-spline curve point
   * that corresponds to the left endpoint of the e-piece segment.
   * \param  p1 Index  of the  b-spline curve  segment containing  the
   * point at parameter \e t1.
   * \param  p2 Index  of the  b-spline curve  segment containing  the
   * point at parameter \e t2.
   * \param u  A parameter value  identifying an e-piece point  on the
   * e-piece segment delimited  by the points at parameters  \e s1 and
   * \e s2.
   * \param c  An index identifying the  c-segment on the left  of the
   * e-piece point at parameter \c u.
   * \param cp1 Array with the  LP matrix column indices corresponding
   * to the control  points of the b-spline  defining the \f$p_1\f$-th
   * piece of the curve.
   * \param cp2 Array with the  LP matrix column indices corresponding
   * to the control  points of the b-spline  defining the \f$p_2\f$-th
   * piece of the curve.
   * \param nl  Array of Cartesian  coordinates of outward  normals to
   * the  supporting  lines of  the  lower  envelope segments  of  the
   * channel.
  ...
  CurveBuilder::insert_linear_terms_of_epiece_point_bounds(
                                                           ...
                                                           const double u ,
                                                           const size_t c ,
                                                           const std::vector< std::vector< size_t > >& cp1 ,
                                                           const std::vector< std::vector< size_t > >& cp2 ,
                                                           const std::vector< std::vector< double > >& nl ,
                                                           const std::vector< std::vector< double > >& nu
                                                          )
  {
    ...
    if ( p1 == p2 ) {
      //
      // Endpoints of the e-piece are in the same curve segment.
      //

      const double c0 = onesixth * ( ( 1 - u ) * ( 1 - s1 ) + u * ( 1 - s2 ) ) ;
      const double c1 = ( 1 - u ) * ( ( ( -2 + 3 * s1 ) * onesixth ) + ( p1 + 4 - t1 ) )
                      +       u   * ( ( ( -2 + 3 * s2 ) * onesixth ) + ( p2 + 4 - t2 ) ) ;
      const double c2 = ( 1 - u ) * ( ( (  1 - 3 * s1 ) * onesixth ) + ( t1 - p1 - 3 ) )
                      +       u   * ( ( (  1 - 3 * s2 ) * onesixth ) + ( t2 - p2 - 3 ) ) ;
      const double c3 = onesixth * ( ( 1 - u ) * s1 + u * s2 ) ;

      for ( size_t v = 0 ; v < 2 ; v++ ) {
        // The upper and lower bounds on the e-piece points must be on
        // or above  the lower envelope  of the (c-1)-th  c-segment of
        // the channel.
        ...
        // The upper and lower bounds on the e-piece points must be on
        // or below  the upper envelope  of the (c-1)-th  c-segment of
        // the channel.
        insert_csegment_constraint(
                                   eqline + 2 ,
                                   c0 ,
      ...
      //
      // Endpoints of the e-piece are distinct curve segments.
      //      
      const double c0 = onesixth * ( 1 - u ) * ( 1 - s1 ) ;
      const double c1 = ( 1 - u ) * ( ( ( -2 + 3 * s1 ) * onesixth ) + ( p1 + 4 - t1 ) )
                      + onesixth * u * s2 ;
      const double c2 = ( 1 - u ) * ( ( (  1 - 3 * s1 ) * onesixth ) + ( t1 - p1 - 3 ) )
                      +       u   * ( ( ( -2 + 3 * s2 ) * onesixth ) + ( p2 + 4 - t2 ) ) ;
      const double c3 = onesixth * ( 1 - u ) * s1
                      + u * ( ( (  1 - 3 * s2 ) * onesixth ) + ( t2 - p2 - 3 ) ) ;
      const double c4 = onesixth * u * s2 ;
...
#ifdef DEBUGMODE
        assert( cp1[ 1 ][ v ] == cp2[ 0 ][ v ] ) ;
        assert( cp1[ 2 ][ v ] == cp2[ 1 ][ v ] ) ;
        assert( cp1[ 3 ][ v ] == cp2[ 2 ][ v ] ) ;
#endif
  ...
  /**
   * \fn void CurveBuilder::insert_rhs_of_sleeve_corners_in_channel_constraints( const size_t eqline , const size_t c , const std::vector< std::vector< double > >& nl , const std::vector< std::vector< double > >& nu , const std::vector< std::vector< double > >& ncsec )
   *
  ...
  {
    const double lcseg2x = ( ( _closed ) ? _lxcoords[ ( c + 1 ) % _nc ] : _lxcoords[ c + 1 ] ) ;
    const double lcseg2y = ( ( _closed ) ? _lycoords[ ( c + 1 ) % _nc ] : _lycoords[ c + 1 ] ) ;
      
    const double ncseg2x = ( ( _closed ) ? ncsec[ ( c + 1 ) % _nc ][ 0 ] : ncsec[ c + 1 ][ 0 ] ) ;
    const double ncseg2y = ( ( _closed ) ? ncsec[ ( c + 1 ) % _nc ][ 1 ] : ncsec[ c + 1 ][ 1 ] ) ;
    ...
    insert_bound( eqline + 3 , Bound::LTE ,        lcseg2x *         ncseg2x +        lcseg2y *         ncseg2y ) ;
    
    insert_bound( eqline + 4 , Bound::LTE , _lxcoords[ c ] *    nl[ c ][ 0 ] + _lycoords[ c ] *    nl[ c ][ 1 ] ) ;
    insert_bound( eqline + 5 , Bound::LTE , _uxcoords[ c ] *    nu[ c ][ 0 ] + _uycoords[ c ] *    nu[ c ][ 1 ] ) ;
    
    insert_bound( eqline + 6 , Bound::GTE , _lxcoords[ c ] * ncsec[ c ][ 0 ] + _lycoords[ c ] * ncsec[ c ][ 1 ] ) ;
    insert_bound( eqline + 7 , Bound::LTE ,        lcseg2x *         ncseg2x +        lcseg2y *         ncseg2y ) ;
  ...
  /**
   * \fn void CurveBuilder::insert_rhs_of_channel_corners_outside_sleeve_constraints( const size_t eqline , const size_t c , const std::vector< std::vector< double > >& nl , const std::vector< std::vector< double > >& nu )
   *
   * \brief Inserts into the matrix associated with the Linear Program
   * (LP) the right-hand side values of the constraints that enforce a
   * channel  corner to  stay outside  the  sleeve. The  type of  each
   * constraint (equality  or inequality:  ==, >= or  <=) is  also set
   * here.
   *
   * \param eqline A counter for the number of constraints.
   * \param c  An index  identifying the  c-segment the  e-piece point
   ...
   */
  void
  CurveBuilder::insert_rhs_of_channel_corners_outside_sleeve_constraints(
                                                                         const size_t eqline ,
                                                                         const size_t c ,
                                                                         const std::vector< std::vector< double > >& nl ,
                                                                         const std::vector< std::vector< double > >& nu
  ...
  {
    insert_bound( eqline     , Bound::LTE , _lxcoords[ c ] * nl[ c - 1 ][ 0 ] + _lycoords[ c ] * nl[ c - 1 ][ 1 ] ) ;
    insert_bound( eqline + 1 , Bound::LTE , _lxcoords[ c ] * nl[     c ][ 0 ] + _lycoords[ c ] * nl[     c ][ 1 ] ) ;

    insert_bound( eqline + 2 , Bound::LTE , _uxcoords[ c ] * nu[ c - 1 ][ 0 ] + _uycoords[ c ] * nu[ c - 1 ][ 1 ] ) ;
    insert_bound( eqline + 3 , Bound::LTE , _uxcoords[ c ] * nu[     c ][ 0 ] + _uycoords[ c ] * nu[     c ][ 1 ] ) ;

    insert_bound( eqline + 4 , Bound::LTE , _lxcoords[ c ] * nl[ c - 1 ][ 0 ] + _lycoords[ c ] * nl[ c - 1 ][ 1 ] ) ;
    insert_bound( eqline + 5 , Bound::LTE , _lxcoords[ c ] * nl[     c ][ 0 ] + _lycoords[ c ] * nl[     c ][ 1 ] ) ;

    insert_bound( eqline + 6 , Bound::LTE , _uxcoords[ c ] * nu[ c - 1 ][ 0 ] + _uycoords[ c ] * nu[ c - 1 ][ 1 ] ) ;
    insert_bound( eqline + 7 , Bound::LTE , _uxcoords[ c ] * nu[     c ][ 0 ] + _uycoords[ c ] * nu[     c ][ 1 ] ) ;
  ...
  CurveBuilder::compute_channel_corners_outside_sleeve_constraints( size_t& eqline )
  {
    ...
    for ( size_t c = 1 ; c < _nc ; c++ ) {
      ...
      // Compute the index of the  curve segments corresponding to the
      // breakpoints of  the e-segment  containing the curve  point \c
      // p(t). Furthermore, compute the parameter  values \c s1 and \c
      // s2 corresponding to the endpoints of the e-segment containing
      // \c p(t).
      
      size_t p1 , p2 ;
      double s1 , s2 , t1 , t2 , u ;
      ...
      if ( s == 0 ) {
        ...
        t1 = p1 + 3 + s1 ;
        t2 = p2 + 3 + s2 ;
        u = ( s - twothird ) / onethird ;
      }

      // Compute  the  column indices  of  the  linear program  matrix
      // corresponding to  the four control points  defining the p1-th
      // segment of the b-spline curve.
      ...
      // Compute  the  column indices  of  the  linear program  matrix
      // corresponding to  the four control points  defining the p2-th
      // segment of the b-spline curve.
      ...
      // Compute the  column indices of  the linear program  matrix of
      // the values  of the  second difference bounds  associated with
      // the p1-th segment of the b-spline curve.
      ...
      for ( size_t j = 1 ; j < 3 ; j++ ) {
        for ( size_t l = 0 ; l < 2 ; l++ ) {
          for ( size_t v = 0 ; v < 2 ; v++ ) {
            sd1[ j - 1 ][ l ][ v ] = compute_second_difference_column_index( p1 , j , l , v ) ;
          }
        }
      }
      ...
      for ( size_t j = 1 ; j < 3 ; j++ ) {
        for ( size_t l = 0 ; l < 2 ; l++ ) {
          for ( size_t v = 0 ; v < 2 ; v++ ) {
            sd2[ j - 1 ][ l ][ v ] = compute_second_difference_column_index( p2 , j , l , v ) ;
          }
        }
      }
      ...
      // ------------------------------------------------------------
      //
      // Process nonlinear terms of Constraint (3b).
      //
      // ------------------------------------------------------------

      // Nonlinear terms of
      //
      // \f$( 1 - u ) \cdot \stackrel{e}{\sim}^p( s_1 ) + u \cdot \stackrel{e}{\sim}^p( s_2 )\f$\sim
      ...
      // ------------------------------------------------------------
      //
      // Process linear terms of Constraint (3b).
      //
      // ------------------------------------------------------------
      ...
      insert_rhs_of_channel_corners_outside_sleeve_constraints(
                                                               eqline ,
                                                               c ,
                                                               nl ,
                                                               nu
  ...
  CurveBuilder::set_up_structural_variables( glp_prob* lp ) const
  {
    ...
    if ( _closed ) {
      size_t s = compute_barycentric_coordinate_column_index( 0 ) ;
      std::stringstream ss ( std::stringstream::in | std::stringstream::out ) ;
      ss << "st" ;
      glp_set_col_name( lp , int( s ) + 1 , ss.str().c_str() ) ;
      glp_set_col_bnds( lp , int( s ) + 1 , GLP_DB , 0.45 , 0.55 ) ;
    }
    else {
      size_t s = compute_barycentric_coordinate_column_index( 0 ) ;
      std::stringstream ss1 ( std::stringstream::in | std::stringstream::out ) ;
      ss1 << "st" ;
      glp_set_col_name( lp , int( s ) + 1 , ss1.str().c_str() ) ;
      glp_set_col_bnds( lp , int( s ) + 1 , GLP_FX , 0.5 , 0.5 ) ;
      
      size_t e = compute_barycentric_coordinate_column_index( 1 ) ;
      std::stringstream ss2 ( std::stringstream::in | std::stringstream::out ) ;
      ss2 << "en" ;
      glp_set_col_name( lp , int( e ) + 1 , ss2.str().c_str() ) ;
      glp_set_col_bnds( lp , int( e ) + 1 , GLP_FX , 0.5 , 0.5 ) ;
    }
